...
def ajax_contract_money(request):
    ...
    whole_money_2017 = ZhbCdProtoinfo.objects.filter(signdate__regex='^2017').aggregate(
        whole_money=Sum('signtotalamt'))
    whole_money_thisyear = ZhbCdProtoinfo.objects.filter(signdate__regex=this_year).aggregate(whole_money=Sum('signtotalamt'))
    whole_money_thisyear =  "{:,}".format(float(whole_money_thisyear['whole_money']) + float(whole_money_2017['whole_money']))
    mm = []
...
def ajax_in_year(request):
    ...
    jj_2017 = ZhbCdLoanInfo.objects.filter(apply_date__regex='^2017').count()
    hp_year = ZhbCdLoanCreditInfo.objects.filter(Q(final_primary_opinion='01') & Q(apprvfinaltime__regex=this_year)).count()
    hp_2017 = ZhbCdLoanCreditInfo.objects.filter(
        Q(final_primary_opinion='01') & Q(apprvfinaltime__regex='^2017')).count()

    in_year = {'jj_year': jj_year + jj_2017,  'hp_year': hp_year + hp_2017}
    in_year = {dfdf:10}
...
city_list = ['安康','宝鸡','保定','滨州','成都','东营','贵阳','汉中','杭州','合肥','济南','济宁','昆明','拉萨','兰州',
             '临沂','柳州','南京','南宁','南通','宁波','钦州','青岛','上海','石家庄','苏州','台州','泰安','天津','潍坊',
             '无锡','西安','西宁','咸阳','襄阳','烟台','银川','玉林','镇江','郑州','淄博','遵义', '重庆', '长沙']
